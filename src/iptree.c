...
iptree_node_t *iptree_create () {
    iptree_node_t *root = (iptree_node_t *)calloc(1, sizeof(iptree_node_t));
    root->prefix = 0;
    root->mask = 0;
    root->l = NULL;
    root->r = NULL;
...
iptree_node_t *create_prefix (uint32_t ip, uint32_t mask, char* data) {
    iptree_node_t *node = (iptree_node_t *)calloc(1, sizeof(iptree_node_t));
    node->prefix = ip & mask;
    node->mask = mask;
    node->l = NULL;
    node->r = NULL;
